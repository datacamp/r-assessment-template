#!/usr/bin/env python3
"""
SQL BlanksChallenge Validator

Validates BlanksChallenge exercise markdown structure for SQL coding items.

Usage:
    python sql_coding_validator.py <exercise_markdown_file>
    python sql_coding_validator.py /tmp/exercise_to_validate.md
"""
...
def parse_single_item(content: str) -> Tuple[dict, List[str]]:
    ...
    # Extract sections - SQL uses {sql} for code1 and {python} for pre_challenge_code
    sections = {
        "context": r'`@context`\s*\n(.*?)(?=`@|\Z)',
        "code1": r'`@code1`\s*\n```(?:sql|\{sql\})\s*\n(.*?)```',
...
def validate_yaml_block(yaml_content: str) -> Tuple[List[str], List[str]]:
    ...
    if unit_match:
        ...
        if not re.match(r'^[a-z0-9]+(-[a-z0-9]+){1,3}$', unit_value):
            warnings.append(f"unit '{unit_value}' should be kebab-case (e.g., 'sql-aggregations', 'query-joins')")
...
def validate_code1_content(code1: str) -> Tuple[List[str], List[str]]:
    """Validate code1 content (no comments, SQL-specific checks)."""
    errors = []
    warnings = []
    ...
    # Check for lowercase SQL keywords (should be UPPERCASE)
    sql_keywords = [
        'select', 'from', 'where', 'group by', 'order by', 'having',
        'join', 'inner join', 'left join', 'right join', 'full join',
        'on', 'as', 'and', 'or', 'not', 'in', 'between', 'like',
        'is null', 'is not null', 'distinct', 'union', 'limit', 'offset',
    ...
    for keyword in sql_keywords:
        # Look for keyword as whole word (not part of column name)
        pattern = r'\b' + keyword + r'\b'
        if re.search(pattern, code1):
            lowercase_found.append(keyword.upper())
    
    if lowercase_found:
        warnings.append(f"SQL keywords should be UPPERCASE: {', '.join(lowercase_found[:5])}" + 
                       ("..." if len(lowercase_found) > 5 else ""))
    
    # Check for missing AS in aliases (common pattern: name city_name vs name AS city_name)
    # This is a heuristic check for potential missing AS keywords
    
    # Check for missing semicolon
    if not code1.rstrip().endswith(';'):
...
def main():
    """CLI entry point."""
    if len(sys.argv) < 2:
        print("SQL BlanksChallenge Validator")
        print("")
        print("Usage:")
        print("  python sql_coding_validator.py <markdown_file>")
        print("")
        print("Example:")
        print("  python sql_coding_validator.py /tmp/exercise_to_validate.md")
        sys.exit(1)
