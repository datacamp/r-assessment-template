#!/usr/bin/env python3
"""
R BlanksChallenge Validator

Validates BlanksChallenge exercise markdown structure for R coding items.

Usage:
    python r_coding_validator.py <exercise_markdown_file>
    python r_coding_validator.py /tmp/exercise_to_validate.md
"""
...
def parse_single_item(content: str) -> Tuple[dict, List[str]]:
    ...
    # Extract sections - support both ```r and ```{r}
    sections = {
        "context": r'`@context`\s*\n(.*?)(?=`@|\Z)',
        "code1": r'`@code1`\s*\n```(?:r|\{r\})\s*\n(.*?)```',
...
def validate_yaml_block(yaml_content: str) -> Tuple[List[str], List[str]]:
    ...
    if unit_match:
        ...
        if not re.match(r'^[a-z0-9]+(-[a-z0-9]+){1,3}$', unit_value):
            warnings.append(f"unit '{unit_value}' should be kebab-case (e.g., 'dplyr-wrangling', 'tidyverse-pipes')")
...
def validate_code1_content(code1: str) -> Tuple[List[str], List[str]]:
    """Validate code1 content (no comments, R-specific checks)."""
    errors = []
    warnings = []
    ...
    # Look for = used for assignment (not in function args or comparisons)
    # Pattern: variable = value (not inside function call)
    assignment_equals = re.findall(r'^\s*[a-zA-Z_][a-zA-Z0-9_.]*\s*=\s*', code1, re.MULTILINE)
    if assignment_equals:
        warnings.append("R convention: use '<-' for assignment instead of '='")
...
def main():
    """CLI entry point."""
    if len(sys.argv) < 2:
        print("R BlanksChallenge Validator")
        print("")
        print("Usage:")
        print("  python r_coding_validator.py <markdown_file>")
        print("")
        print("Example:")
        print("  python r_coding_validator.py /tmp/exercise_to_validate.md")
        sys.exit(1)
