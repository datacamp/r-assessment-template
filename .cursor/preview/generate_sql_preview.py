#!/usr/bin/env python3
"""
Generate SQL Coding Exercise Preview

Reads the HTML template and injects exercise data to create a preview.

Usage:
    python generate_sql_preview.py <exercise_markdown_file>
    python generate_sql_preview.py .cursor/tmp_items/exercise_to_validate.md
"""
...
def extract_exercise_data(markdown_content: str) -> dict:
    ...
    # Extract sections (SQL uses {python} for pre_exercise/sct, {sql} for sample/solution)
    sections = {
...
def generate_preview(exercise_file: Path, output_file: Path = None) -> Path:
    ...
    template_file = script_dir / "sql_coding_preview.html"
...
def main():
    if len(sys.argv) < 2:
        print("Generate SQL Coding Exercise Preview")
        print("")
        print("Usage:")
        print("  python generate_sql_preview.py <exercise_markdown_file>")
        print("")
        print("Example:")
        print("  python generate_sql_preview.py .cursor/tmp_items/exercise_to_validate.md")
        sys.exit(1)
